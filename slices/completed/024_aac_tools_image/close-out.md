...
Run: 2026-09-20 13:39 → 16:56 · 7 phases · 4 bail-outs · 1 test round · doc phase done · $97.75
(planner 22 %, research 3 %, rework 6 %)
...
The estate gets one `aac-tools` image, built in ArgoCDTools, carrying the two
architecture-as-code commands a repo's checkout runs. `gen-architecture` is HelmCharts' generator
ported whole and adapted to the deploy-repo layout: one stage per run, `--stage` and `--producer`
both required, the chart rendered the way Argo's repo-server renders it, the artifact written to
`docs/architecture/<producer>.yaml`. `arch-validate` is the federation's canonical validator,
shipped byte for byte rather than copied again. The ids are the same ids HelmCharts mints, and the
slice proved it against the live published dataset for KubeCoder's prd subset — 9 elements and 16
relations, id for id, `producer` among the three fields that differ.

Around it: ArgoCDTools is laid out one folder per image, each its own kaniko context, with a
`kc project` component and a Jenkins stage per image; KubeCoder's toolchain catalog gains the entry
that makes the image selectable; the register states image pins by provenance — third-party
scanners and validators by digest, our own images not — and the root-rotation records now name nine
out-of-repo copies of the CA root, two of which no record had ever named.

None of it is running yet. The image has never been built by CI, no environment has selected the
toolchain, and no deploy repo is registered as an architecture producer; that last is slice 014's,
and the first two are the operator's, below.

## Outstanding actions

Focus: A1 comes first and gates the rest — an environment that selects the toolchain before
ArgoCDTools has published the image fails to start at all. A3-A5 are four repos sitting on unpushed
commits; A2 is the one acceptance item no green build could earn, and it needs A1 and a deploy
behind it.
...
P4's done-record said nothing landed in /work/KubeCoderDeploy and that ArgoCDTools was the only repo this slice changed; that is no longer true of the slice as a whole.
...
Focus: seven phases, nothing appended, and all four bail-outs in the run belong to P6 — three
blocked and one lost_work on what was meant to be a one-line register edit. P6 is also where the
surprise came from: sent to renumber the CA-root inventory from six to seven, it gated the claim
against the tree and found nine, two of them copies no record had ever named.
...
Focus: B16 first — the run's only major, and it fires at every pod start of this environment from
the next restart. Then B5: the generator keys a whole artifact to the wrong namespace, green and
with no gap line, for any deploy repo whose chart name differs from its registry directory. Fifteen
live, twelve witnessed; eight sit in ArgoCDTools where the image was built, the rest in HelmCharts,
...
Focus: none. Every question this slice raised — the tag rule, the port's width, whether HelmCharts
moved, where the catalog entry lived — was put to the operator and ruled during the run.
...
Focus: S2 is the one that changes another slice — an Ansible-led slice cannot write
`Target: aac-tools` today, and both 014 and 025 plan against this repo. S1 wants a decision: the
IaC/ArgoCDTools job now publishes two images and has still never run the suite. Of the six live,
S3 (closed by the doc phase), S4 and S6 are witnessed; S1, S2 and S7 are read.
