...
- **Backup round-trip**: the CronJob produces a dump file in cloud storage. Operator decrypts with the age private key, restores into a scratch CNPG cluster on `wrkdevk8s`, app starts against the restore and reads its rows.
...
- **`wrkdevk8s` (the chart-development cluster) does not get this substrate.** Helm-chart iteration against `wrkdevk8s` continues to use chart-internal Postgres pods for dev simplicity, or points at an out-of-cluster Postgres on the workstation. Substrate-coupled charts that need a real CNPG behave differently in `configs/dev` vs `configs/prd`; this asymmetry is acceptable for chart development and documented when the first such chart migrates.
