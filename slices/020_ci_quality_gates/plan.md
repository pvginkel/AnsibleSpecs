...
- Ruling (2026-09-15, plan review r1 A1 — kubeconform sees nothing), operator: "Agree" — kubeconform runs in a sibling container that sees only host paths, so a wrongly mounted manifest directory validates zero files and passes. The chart gate must make sure kubeconform actually receives the rendered manifests, and a kubeconform run that validates zero resources fails the build.
...
- **`helm lint` with chart defaults fails 3 of 40 charts** — `media` (nil pointer), `mosquitto` (configmap parse error + missing dependency), `storage` (invalid multi-doc YAML). All three are live prd releases and render with their deployed values (`configs/prd/<chart>/prd/values.yaml`). With those values `media` and `mosquitto` pass lint (default-value artifacts); `storage` still fails lint (its cronjob template's separator, see the Q1 ruling) — `helm template`/`upgrade` tolerate it, `helm lint` does not. `mosquitto`'s chart dependency is not vendored — fetched over the network on each render.
...
- **HelmCharts.** Jenkinsfile (`githubPush()` trigger) selects releases by path diff (`charts/<chart>/.*`, `configs/prd/<chart>/.*`, `terraform-modules/.*`, `_providers/.*` via `utils.hasChanges`) plus a live-digest recheck, and deploys with `helm upgrade --install`. A Jenkinsfile-only push deploys nothing by diff; a change under `charts/<chart>/` redeploys that chart's releases. Deploy selection lives in HelmCharts' own Jenkinsfile — the gate belongs there, not in JenkinsPipelineUtils. `tools/deploy/deploy_cli template` renders a release through the same helm invocation the deploy uses — `_helm_invocation` (`tools/deploy/deploy_cli/helmops.py:173-193`) adds `global.environment`, the chart's post-renderer and the release's `helm_args` — not plain `helm template`. No lint, schema, kubeconform or unittest exists anywhere (HelmCharts, Charts, ArgoCDDeploy, KubeCoderDeploy).
...
Every prd release passes the gate once this phase fixes two chart defects. Checked 2026-09-15 with each release's prd values and no digest `--set`s: all 43 renderable releases render, the three with a post-renderer (`mosquitto`, `grafana`, `prometheus`) also pass kubeconform strict through it, and the only failures are these two.
...
- kubeconform runs from an image pinned by digest, in a sibling container the job's agent starts through the host docker socket (`Ansible/support/iac-agent/bin/jenkins-agent-launch.sh:59-68`; `iac` is itself a `docker run`, `Ansible/support/iac-agent/bin/iac:46-52`), so no iac image change is needed. Kubernetes 1.35 schemas are published at kubeconform's default schema location (checked 2026-09-15); the custom resources in rendered output (e.g. `ExternalSecret`) have none there.
- That container sees host paths only. The agent's workspace (`/home/jenkins/agent`, `jenkins-agent-launch.sh:15`) is not a host mount, and the clone each `iac -c` renders from is private to its container (`iac:46-52` mounts only the secrets file and helper scripts). A bind mount of either path gives kubeconform an empty directory, and it exits 0 having validated nothing. The gate must actually get the rendered manifests to kubeconform, and a kubeconform run that validates zero resources fails the build. No healthy release renders zero resources: every release the loop gates has a chart (prd has no chart-less release; the Argo-reconciled entry never reaches the loop, `tools/chart_tools/resolve_helm_args.py:236-237`), and every local chart's templates emit built-in kinds.
