...
- Ruling (2026-09-14, R5/V12 — P4 executor question): the operator chose "Split k8s play, serial 1", presented as: "Move the k8s leaf into its own last play with serial: 1, so a not-ready node stops the roll there. Other groups are unaffected. Cost: one wedged k8s node skips the later k8s nodes' renewal that week, and each leaf still gets two weekly attempts before it lapses. This reverses the playbook's documented 'no serial' for k8s only." Rejected: a control-plane peer `/readyz` precheck before restarting kubelite, and amending V12 so an un-serialised play only turns red.
  - **Why it came up.** P4 r1 found on ansible-core 2.20.5 that under `throttle: 1` a failed handler does not stop the other hosts in an un-serialised play, whether `any_errors_fatal` is unset, task-level or play-level. So `playbooks/renew-internal-tls.yml` (one play over `proxmox:k8s:openbao`, no `serial`) kept restarting kubelite past a not-ready node. `site-k8s.yml`'s `serial: 1` apply already stops.
  - **What it means for the playbook.** The k8s leaf leaves the shared play for a separate play over `k8s` with `serial: 1`, placed last so a fully failed k8s play cannot cost pveproxy or OpenBao their renewal. The proxmox and openbao leaves keep today's un-serialised play and its reasoning. Both plays keep `force_handlers: true`. The playbook's "No any_errors_fatal and no serial" comment is rewritten to say why k8s is the exception.
