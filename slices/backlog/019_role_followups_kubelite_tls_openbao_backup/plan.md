...
- Settled by the session and shown to the operator in the refinement, not objected to (2026-09-14):
  ...
  - **R4.** In a check-mode run the kubelite restart announces itself as a change on each node it would restart, the way `microk8s/tasks/join.yml` announces a would-be join. Check mode still never restarts anything. The same announcement goes on the four older command/shell restart handlers with the same blind spot, in the same phase: microk8s `Restart microk8s` and `Rollout-restart coredns`, and microceph `Restart microceph OSD daemon` and `Restart microceph MDS daemon`.
