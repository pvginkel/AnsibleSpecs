...
# 031 — Image pins by tag, and the tracking-tag label decides what registry-cleanup may delete

**Major.** On 2026-09-25 registry-cleanup's GC deleted the digest KeycloakDeploy pinned; after a
power cut Keycloak could not pull its image and took DHCP down with it. The operator asked to
reconsider digest pins altogether. This slice makes every deploy-repo pin of an in-registry image
a per-build tag (matrix images included), makes the image's `tracking-tag` label, not the tag's
name, decide which tags are build history, and settles the paused cleanup job.

## What is being requested and why

Follow-up to the 2026-09-25 DHCP outage (epic EPIC-5; report and plan in AnsibleSpecs
`handovers/dhcp-outage-2026-09-25/`). The operator asked to pick up DI-8 together with ANS-125
("Can you pick up DI-8 together with ANS-125.") and, over the triage of 2026-09-26, ruled on the
shape recorded below. Subsumes DI-8 and ANS-125. DI-8 is a DockerImages card; DockerImages has no
spec repo of its own, so the slice is Ansible-led. The triage record is AnsibleSpecs
`handovers/triage_2026-09-26.md` and `…_raw.md` (deleted when this slice was filed; see git
history).

The registry-cleanup CronJob is **suspended** (RegistryDeploy `5ccb234`, 2026-09-25), so no image
is deleted meanwhile. Keycloak's `rebuild-at` is 2026-10-01; while the job stays suspended that
rebuild orphans the pinned digest harmlessly.

## Requirements

1. **[Major — DI-8] registry-cleanup must never delete an image a deploy repo pins, and the pause
   is lifted.** Card: "registry-cleanup must never delete an image a deploy repo pins (GC
...
2. **[Decision — ANS-125] Deploy repos pin tags, never digests.** Operator: "I didn't realize
   ...
   D53."
3. **[Decision — ANS-125, operator's extension] Every matrix build gets a per-build tag.**
...
4. **[Minor — C3, superseded by the operator's ruling] The image's label, not the tag's name,
   ...
   fallback. The finding it replaces (session's wording): "registry-cleanup counts any tag ending
   in a dash and a number as an old build number. So it treats `node-24` and `jdk-21` as build 24
   and build 21, not as named tags."
5. **[Minor — C2] `kube-coder-tunnel-reclaim:latest`.** Session's wording, folded in by the
   operator ("Go"): "**KubeCoderDeploy runs `kube-coder-tunnel-reclaim:latest`,** a moving tag
   that nothing pins."

## Q&A and operator rulings

- **Standing decision overruled (requirements 3 and 4).** DockerImages
  `docs/registry-management/version-poller-redesign.md` §4, "Tag scheme (enforced by `kaniko2`)":
  "Builds push exactly one of: … **one tag** — a version/matrix tag such as `1.35.5` or
  `26.5.1-postgres-health-ispn`. A single tag is **by definition** the tracking tag. Anything else
  is an error. **No exotic schemes.**" and "The mirror classifier used by both tools: a tag is
  **versioned** (numbered history) iff it matches `^\d+$` or `^.+-\d+$`; everything else is a
  **tracking tag**". The operator's label ruling (requirement 4) replaces both: the plan moves
  §4 (and wherever else the design doc and argo-cd D53 state them) to match.
- **Knowing what's deployed is out of scope** (requirement 1). The earlier decision the operator
  refers to is the 2026-08-16 retention model (argo-cd D47 and its scope note; DI-5's comments).
- **Registry analysis is the operator's.** Operator: "I assume at some point I need to do a full
  analysis of what's in the registry and decide what to delete by what rules. Make sure there's
  an operator actions card for that, if there isn't one already." Filed as Operator Action DI-9;
  not this slice's work.
- **Outside (upstream) images pinned by digest** — its own card in ANS intake (ANS-139), not this slice
  (operator, "Go" to "with outside images as a separate card").
- **Open: who lifts the pause.** Asked at triage, unanswered: "A push to RegistryDeploy deploys
  automatically, so if the slice removes `suspend: true` itself, the first run that actually
  deletes images is the slice's." The session proposed stopping at a live dry-run shown to the
  operator; that proposal is not a ruling. Note the operator's "I haven't yet enabled auto
  cleanup".

## Notes for the planner (session observations, unverified beyond what is said)

- **A pin the continuous rebuilds do not reach.** Live on 2026-09-25, `argocd-prd` ran
  `registry:5000/webhook-relay:2485` while `fieldnotes-prd` ran `webhook-relay:2539`:
  `DockerImages/webhook-relay/deploy-pins.json` lists only FieldnotesDeploy, not ArgoCDDeploy.
  DI-8 names `webhook-relay:2485` as one of the tags "5–6 builds from deletion". The operator's
  premise for requirement 1 is that rebuilds keep every pin fresh; this is a case where they do
  not.
- **Overlapping slices.** Slice 029 (HelmCharts decommission) touches the version-poller; slice
  030 retires `modern-app-dev-playwright`, one of the eight matrix images.
- **Related cards.** DI-5 (retention policy, Later) holds the registry-management history and the
  still-open TTL/cap numbers; DI-3 (Later) asks registry-cleanup to honour a `valid-until` expiry
  label.

## Source material

### Triage evidence and chat
...
#### Session (Claude), after the triage check found two collisions with version-poller-redesign.md §4

> (Asked three questions: who lifts the pause; overrule "one tag per matrix build"; overrule the `^.+-\d+$` classifier.)

#### Operator

> As for what counts as a build number. I feel the now obvious answer is that we just make this explicit using a label. That solves a whole slew of problems. It does need an alternate Kaniko invocation and a cross repo migration. Can you give me your opinion on this approach?

#### Session (Claude)

> Agreed, with one refinement: the label already exists. `kaniko2` stamps `org.webathome.poller.tracking-tag` on every image it builds; a tag is tracking only if its name equals that label on the image it points to, every other tag is build history. `kaniko2` takes an explicit `trackingTag:` (derivation from tag shapes stays the default), so a matrix build may push two tags. Images without the label are never cleaned, so the migration can be gradual. The version-poller already falls back to the label for `node-24`/`jdk-21` (`poller.py:127`). The label does not say what is deployed; DI-8's in-use read would still be needed for that.

#### Operator

> I decided earlier that knowing what's deployed is out of scope. If our continuous rebuilds work, there will always be a fresh image. And I haven't yet enabled auto cleanup, so it's not yet a problem.
>
...
>
> Let's go with this then. Proceed with the triage.
