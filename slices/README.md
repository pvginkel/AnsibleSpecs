...
| [postgres-cluster-substrate](postgres-cluster-substrate.md) | pending (dev-first validation, then prd HA) | helm-tf-deploy-harness (done), zfs-dataset-provider (done), backup-collector (done) | (shared CNPG Postgres for persistent app DBs — non-HA on dev, 3-instance sync on prd; per-app DB/role/Secret via TF) |
