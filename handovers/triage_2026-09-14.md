...
- Ruling: I don't feel like fixing this. It's a known gap. I have bigger ones. I would like the following. Document that we should have these controls, but that they're not in place. Make the documentation honest. Then file a card in the Later list (a clean card) to implement the controls.
...
- Ruling: It's known that we need to fix the generated AaC part of HelmCharts. I don't yet know how. It's part of that wrap up yes. So, I'm gonna say park it with Argo CD residuals is fair.
...
- Category: undetermined (suspected Invalid) — "If neither the Friday cron nor a manual run happens before Sep 10, the pveproxy leaves on pve/pve1/pve2 lapse"
...
- Research: were the six leaves re-issued — their current notAfter, or the result of the 2026-09-04 iac-scheduled-certs build?
- Ruling: Scheduled certs is running successfully. I'd say close and if something goes wrong again, I'll reraise.
...
- Category: undetermined (suspected Invalid) — "Worth a look next time Jenkins is reachable."
...
- Research: did iac-on-push pass for b7de205, or for a later `main` commit?
- Ruling: Close. I think this is done now. You could still check the JENKINS_TOKEN. We should have that as a secret in this environment.
