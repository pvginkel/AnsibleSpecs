...
- **TLS**: per-node leaf cert; SANs cover the node's short hostname, FQDN, and `secrets.home` (the VIP). Issued by the homelab `step-ca` via the JWK provisioner, driven by the `internal_tls` Ansible role; 47-day validity, re-issued by the role once remaining validity drops below threshold — but only on an applying run, and no scheduled job applies these leaves today (see "Internal TLS / homelab CA" below). See "Internal TLS / homelab CA" below for the broader CA design.
...
- **Two issuance paths from a single CA, by consumer location**:
  ...
  - **VM consumers** use a reusable Ansible role, `internal_tls`, driving step-ca's **JWK** provisioner via `step ca certificate` / `step ca renew`. The role installs `step-cli`, pulls the JWK provisioner password from ansible-vault into a `mktemp` file for the duration of the task, issues or renews, installs the cert with the caller-specified path / mode / owner, and notifies the caller's reload handler. Re-issue is threshold-gated (default 14 days remaining) and **has no scheduled driver**: iac-scheduled-drift is `--check`-only, so it can report a due renewal but never sign one, and the leaves are converged for real only by a hand-started iac-apply — across three playbooks (`site.yml` for the pveproxy leaves, `site-k8s.yml` for the kube-apiserver SNI leaves, `site-openbao.yml` for the OpenBao listener leaves). SSH host certs are the exception: `iac-scheduled-certs` renews those weekly for real. Closing the same gap for the X.509 leaves is Trello card 737.
