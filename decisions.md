...
- **Cluster changes are serialized.** Never two k8s or Ceph nodes disrupted at once. What gets serialized is the step that takes a node out of service — reboot, upgrade, rebuild, a control-plane restart — not the file write that arms it; a step that disrupts workloads drains and cordons around itself. `serial: 1` on the play holds that line but is not the line itself: it also ends the play at the first failed or unreachable host and leaves the rest untried (a batch in which every host failed stops the run, and `max_fail_percentage` does not lift it). That is right for a sequential roll and wrong for a job whose hosts are independent — certificate renewal, where one wedged host must not cost the others their renewal, so `playbooks/renew-internal-tls.yml` renews the pveproxy and OpenBao leaves in a play with no `serial:` that still reds the build when a host fails. There the one-at-a-time limit rides the mutating task instead: `throttle: 1` on the `Restart openbao` and `Reload openbao` handlers. `Restart microk8s kubelite`, the restart that takes a node's control plane down, carries the same `throttle: 1` whatever play notifies it, plus a readiness wait inside that same task — Ansible finishes a handler on every notified host before starting the next, so restart-and-wait split in two would restart every node before waiting on any. A throttle spaces the restarts but cannot stop them: in a play with no `serial:`, a host failing a throttled handler leaves the other notified hosts to take their turn, `any_errors_fatal` or not. So the k8s leaves, whose reload is that kubelite restart, renew in the playbook's own last play under `serial: 1`: a node that does not come back ready stops the roll, at the cost of the later k8s nodes' renewal that run, and because that play runs last, a k8s batch failing outright ends the run without costing the other groups theirs.
...
- **Terraform — declarative resource tier.** Lifecycle decoupled from any single host, declarative API available. VM existence (`bpg/proxmox` provider, lives in this repo's `terraform/`). Per-application durable resources via the homelab provider (Ceph RBD images, CephFS subvolumes, ZFS datasets), the kubernetes provider (static PersistentVolumes, and the Secrets that carry what Terraform mints), and the Keycloak provider (realms, clients, roles). The per-application TF lives in the app's deploy repo, beside its Helm chart — not in this repo.
- **Helm — runtime tier.** Lifecycle == an application release. Deployments, StatefulSets, Services, Ingress, ConfigMaps, application Secrets, PVCs (claimed against TF-owned PVs), HPAs, PDBs. Per-stage values files. Argo CD deploys each app from its own deploy repo ([`argo-cd/`](argo-cd/decisions.md)).

Worked examples:

- A Ceph RBD image outlives every pod that mounts it. TF owns the image; Helm owns the pod that mounts the PVC.
- A Kubernetes namespace lives and dies with the app's Argo CD Application: it is a tracked manifest in the app's own chart, and the app's Terraform never manages it — the PreSync hook is granted no `namespaces` ([`argo-cd/decisions.md`](argo-cd/decisions.md) D25, D33).
...
**Per-application TF lives in the app's deploy repo**, in `terraform/`, with the stage differences in `config/<stage>/*.tfvars`. Argo CD's PreSync hook applies it in-cluster on every sync of the stage, and a failed apply fails the sync ([`argo-cd/design.md`](argo-cd/design.md); argo-cd D30). Each stage has its own state, keyed `argocd/<repo>/<stage>/terraform.tfstate`, in the same state store as this repo's VM tier and separate from its states (argo-cd D32). A deploy repo carries a copy of each module it uses under `terraform/modules/`. Which apps and stages run is the Argo CD registry: ArgoCDDeploy's from the registry switch, HelmCharts' `release.yaml` tree until the operator has run it ([`argo-cd/decisions.md`](argo-cd/decisions.md) D63, D64). HelmCharts' Helm + TF harness — a per-release `infrastructure.tf` and `configuration.tf` run by its deploy CLI ([`slices/completed/helm-tf-deploy-harness.md`](slices/completed/helm-tf-deploy-harness.md)) — deploys nothing; it stays in the archived repo with the three parked apps (argo-cd D60). Provider extensions for Ceph + ZFS resources: [`slices/completed/tf-provider-resource-extensions.md`](slices/completed/tf-provider-resource-extensions.md) (+ [`zfs-dataset-provider`](slices/completed/zfs-dataset-provider/plan.md)).

**Terraform and Ansible never invoke each other** — peer tools sequenced by the operator (or, for an app's own Terraform, by its Argo CD sync). A composite operation that needs both — drain via Ansible, destroy the VM on Proxmox and `terraform apply` to recreate it, reattach + re-converge via Ansible — is documented in the relevant runbook as a sequence of operator commands, never a single playbook that shells out to `terraform`. Keeps responsibility boundaries clean: TF changes are auditable in `terraform plan`; Ansible changes are auditable in `--check --diff`; mixing them blurs both, and a TF failure halfway through an Ansible play is harder to recover from than a TF failure between two playbook invocations.

**Operator-runs-TF rule** (CLAUDE.md) governs this repo's `terraform/` (VMs, host wiring) — those applies happen at the operator's keystroke. An app's Terraform runs in its Argo CD PreSync hook, gated on the sync; Argo CD is the orchestrator for the application deploy ([`argo-cd/decisions.md`](argo-cd/decisions.md) D1, D30).

The pre-Ansible `/work/KubernetesConfig` repo predates this split: it codified bring-up steps (`.microk8s.yaml`, MetalLB IPAddressPools, registry mirror config, procedural install docs) in a third location that today belongs on the Ansible side. The microk8s build-out absorbed its contents into the `microk8s` role and inventory; KubernetesConfig is now **marked for deletion** (it holds secrets) — tracked in [`slices/runtime-secrets-sweep.md`](slices/runtime-secrets-sweep.md) §4. The operator runs their own ingress controller and own container registry as apps of their own (`NginxDeploy`, `RegistryDeploy`) — `core/ingress` and `core/registry` are not enabled.
...
- **TLS**: per-node leaf cert; SANs cover the node's short hostname, FQDN, and `secrets.home` (the VIP). Issued by the homelab `step-ca` via the JWK provisioner, driven by the `internal_tls` Ansible role; 47-day validity, re-issued by the role once remaining validity drops below threshold; the weekly `iac-scheduled-certs` job is what drives that renewal. See "Internal TLS / homelab CA" below for the broader CA design.
...
Rotation of AppRole `secret_id`s and of application secrets lives in the operator's head today; the `runtime-secrets-sweep` slice exercises the flow but doesn't formalise it. Five distinct patterns to capture:
...
- **AppRole `secret_id` whose value lives in a hand-staged k8s Secret** (`eso` on the prd cluster, `eso-dev` on the dev cluster — bootstrap-tier): same `bao write -force`, but the new value is written via `kubectl create secret … --dry-run=client | kubectl apply -f -` into the `openbao-eso-approle` Secret in that cluster's `external-secrets` namespace, followed by `kubectl rollout restart deployment/external-secrets` because ESO reads the AppRole material on pod startup only.
...
- **An application secret the CA it authenticates to also holds** (`kv/eso/prd/kubecoder/<stage>/step-ca-provisioner-password`): reads as the bullet above and behaves like neither half of it. The value decrypts `kubecoder-jwk`'s `encryptedKey` in step-ca's own `ca.json`, so a KV-only `bao kv put` leaves the two sides disagreeing until the blob is re-encrypted under the new password, deployed through `StepCaDeploy` (its chart carries `step-ca-config`), and the step-ca pod rolled — step-ca reads `ca.json` at process start only. The consuming end needs a restart too: the KubeCoder controller takes the value as a start-time env var through a `secretKeyRef`, with nothing hashing that Secret into the pod template, so an ESO refresh never reaches a running controller. Rotate as: write both leaves, re-encrypt and deploy, roll step-ca, roll the controller.
...
Every release that talks to S3 owns its own RGW user, minted by Terraform. The shared `csi-prd` and `csi-dev` users are deleted.

- **Per-release users.** The `s3-storage` Terraform module, copied into each deploy repo that uses it (`terraform/modules/s3-storage`), wraps the provider's `homelab_s3_storage`: one RGW user named after the release namespace (`<chart>-<stage>`), owning exactly the release's buckets, with `max_buckets = -1` so it can never create another. Its keys go into the Terraform-written Secret `s3-credentials` in the release namespace; OpenBao holds no copy. Rotation is per release (`key_rotation`), so blast radius bounds itself. The user follows the namespace, not the cluster: the prd cluster hosts every stage, and each stage's release has its own user.
- **The admin key is not an app credential.** The provider creates users and buckets with the per-cluster RGW admin key in `kv/shared/<cluster>/ceph-rgw/s3`, read by the Argo CD PreSync hook through `argocd-hook-credentials`. The `jenkins` and `iac-agent` AppRoles still hold the grant they had for the HelmCharts deploy, which deploys nothing.
- **CI validation users.** HelmCharts `configs/dev/_ci/` mints one user per app validation pipeline, with the same resource, on the dev cluster's microceph; the operator copies the keys to `kv/shared/validation/<app>/s3` for Jenkins.
- **The one exception: `backup-reader`, fleet-wide and read-only** (slice 022, an operator overrule of per-app-only). The S3 mirror (§"Backup") reads every production-stage bucket. A per-app user reads only its own buckets and can write them, so without the reader the mirror would hold every app's read-write key or the admin key; one reader, granted per bucket, reads them all and can change none. The prd `storage` release creates it with the provider's `homelab_s3_reader`: admin capability `buckets=read` (bucket enumeration through the admin API) and nothing more, `max_buckets = -1`, no bucket of its own. Its keys go into the Terraform-written Secret `backup-reader-credentials` in `storage-prd`. The grant is `homelab_s3_storage`'s `grant_backup_reader`: a bucket policy allowing the reader `s3:ListBucket` and `s3:GetObject`, and it is the bucket's whole policy. The `s3-storage` module sets it for every namespace ending in `-prd`, so no call site sets anything and no new production bucket goes ungranted. The provider takes the reader's name from `HOMELAB_S3_BACKUP_READER`, set only in the prd hook's environment (ArgoCDDeploy `config/prd/values.yaml`, bound by its render gate to HelmCharts' `_providers/clusters.yaml`); on the dev cluster no reader is configured and the grant writes nothing.
...
- **Backup path**: the wrapper authenticates via an AppRole with a read-only export policy — `list`/`read` on the policy catalogue, auth methods, mounts, and the KV-v2 tree, plus `read` on `sys/storage/raft/snapshot` for the snapshot; its one other grant is `update` on `auth/token/revoke-self`, so a login can revoke its own token — assembles the `.tgz`, and POSTs it to the in-cluster `backup-server` (a small Go service in `/work/DockerImages/backup-server/`, deployed by the `storage` app from `StorageDeploy`). The bearer token authorising the upload lives on each srvvaultN as a file (mode 0400, owner openbao), materialised there by the `openbao` role.
...
- **Retention** is enforced server-side by `backup-server`. Each scope's retention is a field of its record in the server's credential store (`credentials.json`, one record per scope holding its upload token and retention; DockerImages `backup-server/src/internal/auth/store.go`), set per scope through the provider's `homelab_backup_credential` (`openbao`: 14, Ansible `terraform/prd/openbao.tf`). After each successful upload it keeps the scope's N newest backups and prunes the rest, each with its metadata file; metadata files do not count toward N. The OpenBao timer does not manage retention.
- **Freshness is watched by `backup-server`, not by the nodes.** Each upload declares how long it stays valid: the wrapper sends `valid_for=52h`. Once the backup has landed in cloud storage, `backup-server` writes `<backup object>.metadata.json` beside it, holding only `{"valid_for":"52h"}`. A stream is scope + file name. It is watched while any of its kept backups declares a validity, and overdue once its newest backup's landing time plus the newest declared validity has passed. `backup-server` reads every scope folder under its remote, whether or not the scope still has a credential: the credential store also authorizes uploads, so watching through it would let a lost store or a removed credential stop a scope's uploads and its alert together. It reads at start, hourly and right after each upload, reads a stream's newest metadata file only when it does not remember it already, and publishes each stream's landing and valid-until times on pod port 8081, inside the cluster only. Production Prometheus scrapes it through the Service annotations and raises two critical alerts in Alertmanager: `BackupOverdue` once a stream passes its valid-until, and `BackupWatcherBlind` once no full read of cloud storage has succeeded for 6 h, including when `backup-server` cannot be scraped at all. 52 h rather than 48 because the timer fires at 02:00 with up to 1 h random delay: one missed night stays quiet, two in a row alert. The followers still exit 0; no node's unit status is the signal. Triage, and retiring a scope (it alerts until its `.metadata.json` files are deleted by hand): Ansible `docs/runbooks/backup-freshness.md`.
...
- **Recovery paths**:
  ...
  - *Whole-cluster loss* (all three down simultaneously, the extreme case): Terraform recreates all three; roles converge into a fresh empty cluster — first node initialises with the same seal key, the others Raft-join. The latest `.tgz` is pulled from cloud storage with `rclone` and decrypted with the age private key from Roboform; `bao operator raft snapshot restore` replays the snapshot into the fresh cluster. The restore replaces cluster state with the snapshot's, so the AppRole credentials and the Shamir recovery-key configuration come back intact — consumers need no credential redistribution, and the Roboform-held recovery keys still work. The backup pipeline's own secret_id comes from a converge after the restore: the first converge had no `backup` AppRole to prove a staged one against, so it installs none. ESO resyncs Kubernetes Secrets; consumers reconnect.
- **Failure domains**: two — Roboform (Shamir recovery shards + age private key + ansible-vault passphrase + the S3 mirror's crypt password and salt) and the cluster-plus-its-backups (Raft data + live seal key on each srvvaultN + age public key on `backup-server` in k8s + the crypt password and salt in OpenBao `eso/prd/storage/prd/s3-mirror` and the `storage-s3-mirror` Secret + the cloud-storage path with the encrypted backups and the S3 mirror). Both must leak for full secret compromise. The S3 mirror's bucket data sits in the second domain alone: rclone crypt's key is symmetric, so the mirror job holds a key that also decrypts, and cluster access plus the Drive login reads the mirror (the cluster also holds a Drive token, in `rclone.conf` on `rclone-backup-pvc`). The age-encrypted uploads, by contrast, need Roboform's private key. Three-domain isolation existed under the Azure design; one domain was deliberately given up to remove the cloud dependency.
...
- **`iac-impl` is rewritten from bash to Python**, in `pvginkel/Ansible` at `support/iac-agent/bin/iac-impl` alongside the rest of the srviac host glue. Python deps land in the same repo's `pyproject.toml` because the iac container bakes its venv from there. One language, one entry point, room for future drift checks.
...
- **What does *not* get its own AppRole.** Ansible-via-iac-impl consumes env + files materialised by the resolver before it ever runs, so it does not need an AppRole of its own. Jenkins (for pipeline secrets) and ESO (for in-cluster secret sync) each retain their own AppRoles + policies; Ansible does not. Neither does anything else running in-cluster: a workload that needs OpenBao material takes it as an ESO-materialised Secret, leaf by enumerated leaf, and stays agnostic to where it came from. An AppRole would bound that workload by a KV prefix; the Secret bounds it by exactly what the ExternalSecret names, and adding a shared credential becomes a commit rather than a policy widening plus a live OpenBao run. srviac resolves its own refs because it is a VM outside Kubernetes with no ESO to lift off — that is the exception's reason, not a pattern to carry inward.
...
- **`step-ca`** (Smallstep), deployed by Argo CD from `StepCaDeploy`.
- **Two issuance paths from a single CA, by consumer location**:
  - **In-cluster consumers** use cert-manager driven by step-ca's **ACME** provisioner, replicating the existing public-cert flow (cert-manager + Let's Encrypt) with a different directory URL. A step-ca `ClusterIssuer` lands in a deploy repo; consumers opt in by annotation on their Ingress. No bespoke Ansible plumbing.
  - **VM consumers** use a reusable Ansible role, `internal_tls`, driving step-ca's **JWK** provisioner via `step ca certificate` / `step ca renew`. The role installs `step-cli`, pulls the JWK provisioner password from ansible-vault into a `mktemp` file for the duration of the task, issues or renews, installs the cert with the caller-specified path / mode / owner, and notifies the caller's reload handler. Re-issue is threshold-gated (default 14 days remaining) and driven weekly by `iac-scheduled-certs` — the same Friday job that renews the SSH host certs, running `playbooks/renew-internal-tls.yml` across every leaf in the fleet, which weekly against a 14-day window gives two attempts before any leaf lapses. The converge playbooks issue the leaves too (`site.yml` for the pveproxy leaves, `site-k8s.yml` for the kube-apiserver SNI leaves, `site-openbao.yml` for the OpenBao listener leaves), but only under a hand-started iac-apply; iac-scheduled-drift is `--check`-only, so it can never sign one. **Drift checks the leaves at 7 days, one certs-job period shorter than the role's 14** (`-e internal_tls_renewal_threshold_days=7` in `Jenkinsfile.iac-scheduled-drift`): a leaf inside its window is the Friday job's business, so drift reds only for a renewal that Friday already missed — the backstop, not the noise. Keep it at the role's threshold minus the certs job's period if either moves.
- **JWK provisioner password is bootstrap-tier**: a single fleet-wide password, encrypted in the Ansible repo via ansible-vault, passphrase in Roboform. Lives alongside the OpenBao seal key, ansible-vault passphrase, and the IaC agent's AppRole credentials in the irreducible-literal set — never moves into OpenBao, because OpenBao's own listener cert is issued by this same path. **Per-provisioner name scoping is wanted and not in place.** `ansible-jwk`'s `ca.json` entry carries an exact-name `options.x509.allow.dns` list (the Proxmox hosts, `srvceph1..3`, the `kubernetes*` and `kubernetes-api*` names) plus an `allow.ip` list, and `kubecoder-jwk` is configured the same way — but step-ca 0.30.2 does not honour per-provisioner name policy from a file-based `ca.json`; it fills those fields only through the remote-management API, which this estate does not use. Both lists are dead configuration: OpenBao's listener leaf, issued through `ansible-jwk`, carries `secrets.home` and `srvvault1.home`, neither of them listed. Until the controls exist, neither password is limited to any set of names. Implementing them is Trello #993.
- **KubeCoder's own provisioner password is the exception, and lives in OpenBao.** KubeCoder drives this CA through a JWK provisioner of its own — `kubecoder-jwk`, separate key and separate password, never the fleet's — and that password is materialised into the cluster by ESO from `kv/eso/prd/kubecoder/<stage>/step-ca-provisioner-password` rather than kept in ansible-vault. The bullet above rules that out for `ansible-jwk`, but its reason is bootstrap ordering: OpenBao's own listener cert is issued through that provisioner, so that password must be available before OpenBao is. KubeCoder is not in the whole-site bring-up order (§OpenBao backup / DR), so the reason does not reach it. Recorded as an explicit exception rather than left to land silently; the bootstrap-tier rule is unchanged and `ansible-jwk`'s password stays out of OpenBao.
...
- **Intermediate key + passphrase: regular Kubernetes Secret**, not delivered via ESO from OpenBao. The chicken-and-egg of OpenBao-needs-cert-from-step-ca-needs-passphrase-from-OpenBao is not worth the marginal security gain when short leaf validity already bounds blast radius. **Materialised at cluster bring-up via a small Ansible role from ansible-vault'd source files**, not committed as plaintext-base64 manifests in a deploy repo — same bootstrap-tier posture as the JWK provisioner password and the OpenBao seal key. Covers all four `step-ca-*` Secrets that currently live in `StepCaDeploy`'s `chart/templates/stage-manifests.yaml` (`step-ca-config`, `step-ca-secrets`, `step-ca-ca-password`, `step-ca-ssh-host-ca-password`); the only non-secret entry in that file is `step-ca-certs` (public certs + SSH host CA pub), which can stay in the chart or move to a ConfigMap. Extraction tracked in the runtime-secrets-sweep slice's §Loose ends; this is what makes that file publishable.
- **Leaf cert validity: 47 days**, all consumers. The `internal_tls` role re-issues a VM leaf when remaining validity drops below 14 days; in-cluster leaves are re-issued by the monthly `certificate-renewer` CronJob. Each leaf's absolute expiry is meant to be published as the Prometheus gauge `internal_tls_cert_not_after_seconds`, and today only the Proxmox and OpenBao leaves get one: the `internal_tls` role writes it to the node-exporter textfile collector and skips the write where that directory is absent — the steady state on the k8s nodes, which run node_exporter as an in-cluster DaemonSet — so the four kube-apiserver SNI leaves (srvk8s1–3, srvk8sdev) have no gauge. Of the hosts that do write it, Prometheus's `homelab-nodes` job scrapes only the Proxmox nodes, and the in-cluster collector for the certbot path is not built. The design is a single Prometheus alert on a stalled renewer at `<10` days remaining — below both the 14-day VM renewal threshold and the in-cluster monthly path's ~17-day steady-state floor, so it signals a real failure rather than flapping every renewal cycle — and **no alert exists on any leaf yet**. The alert rule, the in-cluster metric and the kube-apiserver leaves' metric path are **deferred** (observability is not a current priority); design parked in [`slices/deferred/internal-tls-monitoring.md`](slices/deferred/internal-tls-monitoring.md).
...
- **Ten out-of-repo copies of the same file are in use**, all byte-identical to the Ansible canonical copy. Five are baked into images: `/work/ArgoCDTools/argocd-hook/image/homelab-root.crt` (the Argo CD presync hook image), `/work/ArgoCDTools/aac-tools/image/homelab-root.crt` (the `aac-tools` image), `/work/DockerImages/kube-coder-dev-base/homelab-root.crt`, `/work/DockerImages/kube-coder-arm64-cross-toolchain/homelab-root.crt` and `/work/DockerImages/kube-coder-esp-idf-toolchain/homelab-root.crt`. Five ride deploy repos' charts: `ArgoCDDeploy` `chart/files/homelab-root.crt` (mounted into Argo CD's repo-server so `helm dependency build` trusts `https://charts.home`), `ExternalSecretsDeploy` `chart/files/homelab-root.crt` (the `homelab-root-ca` ConfigMap, argo-cd D59), `JenkinsDeploy` `chart/files/ca/homelab-root.crt`, `KubeCoderDeploy` `chart/files/ca/homelab-root.crt` (KubeCoder's controller CA mount, below) and `NginxDeploy` `chart/files/ca/homelab-root.crt`. HelmCharts' copies (`homelab-root.crt`, `charts/nginx/files/ca/`, and the `charts/jenkins/` and `charts/kubecoder/` symlinks to the first) deploy nothing and stay in the archive. A rotation that updates the in-repo bundle here must update all ten in the same change window, ordered *before* the new root starts appearing in step-ca's `/roots.pem` (otherwise the drift pipeline fires until the others catch up); the five image copies additionally need their images rebuilt to take effect, and each deploy-repo copy reaches the cluster with its app's next sync, which for ArgoCDDeploy's is a manual sync of Argo's own chart (argo-cd D3). Eventual fix is to collapse the duplication — single source via submodule, or build-time fetch from this repo — recorded as a follow-up rather than blocking the next rotation.
- **The KubeCoder controller keeps no copy of its own.** It validates `https://ca.home` when it asks step-ca to sign an environment pod's SSH host certificate, and its base image (`python:slim`) carries stock Debian roots only — so the root reaches it as a chart mount rather than through the image's trust store: `KubeCoderDeploy`'s chart renders its `chart/files/ca/homelab-root.crt` into a ConfigMap and hands the pod the mounted path in `KUBECODER_STEP_CA_ROOT`. A rotation lands there by a chart deploy, with no image rebuild. The controller image did bake its own `/work/KubeCoder/controller/homelab-root.crt` until September 2026; that file is gone and the Dockerfile no longer copies it.
...
- Finish `docs/runbooks/step-ca-root-rotation.md`. It exists and carries the settled half — the full inventory of artefacts a root change moves (canonical copy, nine out-of-repo copies, the images that consume the cert without holding a copy, the parallel four-copy `terraform.rc` set) and the ordering constraint. The step-by-step cutover procedure is still owed, and is blocked on the two mechanism items below.
...
- Decide and implement the deduplication mechanism for the nine out-of-repo copies, or accept the maintenance burden by documenting every path in the runbook.
...
1. **Upgrade first** — bring the cluster to its target microceph LTS channel via `snap refresh`, mons before OSDs, `serial: 1`. Soak under real workload for several days; confirm `HEALTH_OK` and that the apps consuming Ceph are unaffected.
2. **Then rebuild** — drain a node, destroy the VM on Proxmox and `terraform apply` to recreate it, apply baseline + microceph role, reattach the existing OSD disks (passthrough disks, which the Proxmox destroy leaves in place; BlueStore OSDs carry their identity on-disk). Repeat one node at a time.
...
**Track a recent `stable` channel — not pre-release.** Same low-surprise logic as Ceph, but k8s moves fast enough that pinning to the previous LTS strands us behind the addon/CNI and Helm chart ecosystem, so we track a current `stable` minor instead. Never `candidate`/`beta`/`edge` except for a brief, deliberate upgrade soak. Channel pinned per cluster in `group_vars/k8s_{prd,dev}.yml` so dev runs a minor ahead of prd and soaks it before prd moves. Upgrade when the running minor nears EOL, a security fix forces it, or a fix we need lands upstream.

Today **`1.35/stable` is the homelab k8s base version** (prd). Dev runs `1.36/stable`, a minor ahead, to soak the next release before prd moves. The scratch fleet is not currently deployed — its `group_vars` channel pin is inert until the fleet is stood up, so don't read it as a live version. The strict-confinement variants (`*-strict/stable`) are rejected — extra surface for limited benefit on this workload.
...
**A cluster upgrade also has to bump the `kubernetes` Python client in the other repos.** Several consumers reach the cluster API through the `kubernetes` Python client (all via the same `load_incluster_config()` + `get_default_copy()` pattern) and must pin it to the cluster's minor — currently `kubernetes~=35.0` for the 1.35 base: the DockerImages images `dnsmasq-config-generator`, `nginx-configurator`, `infra-statistics`, `certbot` (each in its `requirements.txt` / `Dockerfile`); the `ZigbeeControl` app (`pyproject.toml`); and the Helm deploy tooling in `HelmCharts/tools/requirements.txt` — all realigned to `~=35.0`. The client tolerates n±1 skew, so the safe order is: bump every pin to the new minor (e.g. `~=36.0` ahead of a 1.36 move), rebuild + redeploy the affected images, *then* upgrade the cluster. Noted here because the sequencing is an upgrade-procedure concern even though the pins live outside this repo. Origin: a rebuild with an unpinned client pulled `kubernetes` 36.0.0, whose `auth_settings()` had regressed out of step with `load_incluster_config()`, sent every request unauthenticated, and broke service-annotation DNS.
...
**Revisit after the plan**: [Headlamp](https://headlamp.dev/) (CNCF, modern UI, plugin system) as the replacement. It runs now, deployed by Argo CD from `HeadlampDeploy` (upstream chart), and the dashboard addon is still enabled beside it. Same web-UI workflow, version ownership shifts off microk8s's release cadence onto the operator's Helm flow. `k9s` (terminal UI) and OpenLens / desktop apps are off the table — operator wants browser-based.
...
- For Terraform-provisioned VMs outside the bring-up tier, the per-VM module declares a `homelab_dns_reservation` resource that registers the (hostname, MAC) pair with the dnsmasq sidecar API; the API allocates the IPv4 from `10.1.3.0/24`. Bring-up-tier hosts are static instead (see "MAC addressing" below). `depends_on` on the VM resource orders the reservation before VM create, so the VM's first DHCP request lands on a known reservation. See "MAC addressing" below for the resource shape.
...
| k8s nodes (`srvk8s1/2/3/4`)                                      | vmbr0 + vmbr0 tag=2 + vmbr1 |
| `srviac`, `srvk8sdev`                                            | vmbr0 + vmbr1               |
...
- **dnsmasq reservation as a Terraform resource**: managed VMs register their (hostname, MAC) pair with the dnsmasq sidecar API via a `homelab_dns_reservation` resource inside each per-VM module; the API allocates the IPv4. One apply registers the reservation and creates the VM, in that order; destroy reverses it. The sidecar is deployed with DnsmasqDeploy; the static-hosts list (DnsmasqDeploy `chart/templates/stage-manifests.yaml`) continues to hold operator-curated entries (printers, IoT, network gear, the bring-up tier). The two sets are served side by side: on a hostname collision dnsmasq answers with both addresses. Moving a host between them relies on that overlap; see Ansible `docs/runbooks/static-address.md`. Specs: [`specs/dns-reservation-api.md`](specs/dns-reservation-api.md), [`specs/dns-reservation-terraform.md`](specs/dns-reservation-terraform.md).
- **Ceph nodes, prd k8s nodes and `srviac` are static infrastructure, not dynamic reservations**: `srvceph1/2/3`, `srvk8s1/2/3` and `srviac` opt out of the `homelab_dns_reservation` resource via `static_ip = true` on the `managed-vm` module. Their hostname → IP pairs live exclusively in DnsmasqDeploy's static-hosts (`chart/templates/stage-manifests.yaml`), hand-curated alongside printers/IoT/network gear. The test for the tier is whether the host must be on the LAN while the in-cluster DHCP is down. Whether it hosts bring-up pods is not the test. srvk8s4 (a worker hosting none) and srviac (the break-glass runner) were reservation hosts in the 2026-09-25 outage, which left both without an address for 2h45m. srviac has moved since. **srvk8s4 is the one exception still on a reservation**: moving a joined microk8s node needs a worker re-join, because its kubelet serving cert names its IPs, and that move is deferred. Rationale: the dnsmasq sidecar runs in-cluster behind the registry; the registry container itself depends on Ceph storage to boot, and the cluster nodes that host both the registry and dnsmasq cannot get their own addresses or DNS from a service they themselves are required to bring up. Any chain that puts bring-up-tier addressing behind the dynamic API creates a cold-boot ordering failure. Static IPs (declared per-NIC in `vms.tf`, rendered into the cloud-init template's netplan section) sidestep the cycle entirely. External nameservers (`8.8.8.8` / `8.8.4.4`) are pinned at the host so containerd's image-pull DNS path on these nodes never depends on the cluster being healthy. `srvk8sdev` is dev-tier and stays on the dynamic reservation — it doesn't host registry/dnsmasq pods (dev pulls from external `registry-dev`), so the bring-up cycle doesn't apply.
- **Cloud-init is a first-boot artefact.** Its scope: ansible user, ansible SSH pubkey, pinned ed25519 host key, qemu-guest-agent, and (for hosts with static addresses on any NIC) the initial netplan render. After first boot, Ansible owns drift detection and convergence on these surfaces. The baseline role's netplan task re-asserts `/etc/netplan/50-cloud-init.yaml` from the `network_devices` host_var, which `vms.tf` also reads, so a static-IP change lands on the running host without a rebuild. Proxmox's instance-id is a hash of the cloud-init data, so a Terraform `ipconfig0` change makes the next cold start a "new instance", and cloud-init re-renders netplan from Proxmox's network-data. That data describes net0 only, so the other NICs stay down until a `--tags netplan` run. A change of address uses exactly that: Ansible `docs/runbooks/static-address.md`. cloud-init's network stage stays on, because it also renames the primary NIC to `eth0` on every boot, and keepalived and Calico on the k8s nodes are bound to that name. A microk8s node also has to re-join, because its kubelet serving cert names its IPs. The `managed-vm` module pins `lifecycle.ignore_changes` on `initialization[0].user_data_file_id` on the VM resource so a template edit re-renders the snippet but does not recreate the VM. To pick up a template change on an existing host, rebuild it: destroy the VM on Proxmox, then `terraform apply` recreates it and cloud-init runs on its first boot.
...
- **47-day certs, 14-day renewal threshold.** Same shape as the TLS leaves, schedule included: the weekly `iac-scheduled-certs` job runs `playbooks/renew-host-certs.yml`, and `ssh_host_cert` re-signs when the cert is within the threshold, otherwise no-op. Unlike the TLS leaves, drift is no backstop here: the sign step is a `command` with no check-mode report, so `--check` skips it silently and the certs job's own build is the only signal for a host cert renewal that stopped.
...
- **Concurrency control: per-state lock branches, no host lock.** `terraform-backend-git` takes a `locks/<state-path>` branch per state, so a force-unlock means deleting that branch — unlike the old file-based model, which had no remote lock. The `/var/lock/iac.lock` flock that once serialised `iac` invocations on srviac is removed (Trello #506). As a state guard it was illusory — wrkdev and the KubeCoder pod write the same store and it saw neither — and `flock -w 60` failed on contention rather than queueing, so a collision became a red build. Cross-job serialisation comes from Jenkins instead: the `IaC Agent` node has one executor, so the IaC jobs queue behind one another. Accepted loss: hand-run Ansible on srviac no longer interlocks with a running job (terraform still does, via lock branches).
...
- **From the operator workstation (the carve-out)**: only changes that mutate the Jenkins agent VM itself — first-time bootstrap of the agent, agent VM disk resize, agent VM rebuild or removal (destroyed on Proxmox first, then applied — see below), break-glass when CI is down. The orchestrator cannot orchestrate its own replacement.

Guards against a destroyed VM. **Terraform never destroys a VM, in Jenkins or locally.** There are two rails, listed in the order they act:

- **`lifecycle { prevent_destroy = true }` on the VM resource in the shared `managed-vm` module** (`proxmox_virtual_environment_vm.this`), which every prd VM goes through. It stops the plan, not the apply. A plan that would delete or replace a VM fails with `Error: Instance cannot be destroyed` and `terraform plan` exits 1, so a job stops before `terraform show` and the destroy guard run. That refusal is the protected-VM signal: `iac-scheduled-drift` names each refused VM in its build description rather than reporting it as drift. `terraform/scratch` has its own VM resource and keeps its `-replace` rebuilds.
- **CI destroy guard.** `check-protected-vms.sh` runs against the `terraform/prd` plan JSON in `iac-on-push`, `iac-apply` and `iac-scheduled-drift`. It fails the job on any change that deletes or replaces a prd VM, Terraform's default `["delete","create"]` replace included, with no name list and no override. It is the second rail: while `prevent_destroy` is in the config no such plan reaches it, so it catches one only for a config without that line, such as while it is lifted. `iac-apply` plans, runs the guard and applies that saved plan inside one `iac` invocation, so the plan it checked is the plan it applies.
- **Both rails cover the VM resource only.** A VM's companions (the `homelab_dns_reservation` in the module, the cloud-init snippet outside it) are ordinary resources, and a VM's removal deletes them as planned.

**A prd VM is rebuilt or removed by destroying it on Proxmox first** (`qm destroy` on its PVE node). The next apply's refresh finds the VM gone and recreates it under its pinned `vm_id`, or, once its `vms.tf` entry is gone, forgets it and deletes its companions. The apply job stays the operator's tool for creating and removing VMs; only the destroy itself sits outside Terraform. A Ceph node's OSD disks are passthrough disks, not Proxmox volumes, so `qm destroy` leaves them in place. A VM a failed create left tainted fails every `terraform/prd` plan, the daily drift run's included, until it is destroyed on Proxmox and the apply re-run. Procedures: `docs/runbooks/vm-rebuild.md`, `k8s-rebuild.md`, `openbao.md`, `iac-agent.md`.
...
- **Off-cluster copies of production data**, each on Google Drive under `gdrive-pieter:Homelab Backups`:
  - *OpenBao* — the leader srvvaultN's daily `.tgz` through `backup-server`, scope `openbao`, 14 newest uploads kept (Ansible `terraform/prd/openbao.tf`); age-encrypted to the operator's key, private half in Roboform. `BackupOverdue` fires critical after 52 h without a new backup. Triage: Ansible `docs/runbooks/backup-freshness.md`. See §"OpenBao backup / DR".
  - *`postgres-pas` databases* — the `postgres-backup` CronJob's nightly `pg_dump` of each application database through `backup-server`, scope `postgres-pas`, 90 newest uploads kept (`PostgresPasDeploy` `terraform/main.tf`); the same age key. `BackupOverdue` fires critical for a database after 52 h without a new dump of it; a database dropped on purpose alerts until pruning removes its last dump or its `.metadata.json` files are deleted by hand. Triage: Ansible `docs/runbooks/backup-freshness.md`.
  - *YouTrack* — the `youtrack-backup` CronJob in `youtrack-prd`, nightly: YouTrack writes its own database backup (issues, attachments, the built-in Hub), which the job starts and downloads over the REST API as the `backup` service user (token in OpenBao `eso/prd/youtrack/prd/backup`) and posts through `backup-server`, scope `youtrack`, 30 newest uploads kept (`YoutrackDeploy` `terraform/main.tf`); the same age key. Not the RBD volume: the embedded database cannot be copied while open, and the volume is `ReadWriteOnce`. Its uploads declare no validity, so `BackupOverdue` does not watch it; `YouTrackBackupStale` fires critical after 52 h without a successful run of the CronJob. Restore and drill: Ansible `docs/runbooks/youtrack-restore.md`.
  - *Production-stage RGW buckets* — the `s3-mirror` CronJob in `storage-prd`, nightly: an incremental `rclone sync` of every bucket owned by a `-prd` RGW user, read as `backup-reader` (§"Ceph RGW credentials"), into an rclone crypt remote at `Homelab Backups/s3-mirror`. Decrypted, `current/<bucket>/` is live state and `archive/<bucket>/<run stamp>/` holds what that run replaced or deleted; the 30 newest archive folders per bucket are kept, so history counts changes, not days. A bucket gone from RGW keeps its mirror. Key: the crypt password and salt, in OpenBao `eso/prd/storage/prd/s3-mirror` and in Roboform. `S3MirrorStale` fires critical after 52 h without a successful run. Restore and drill: Ansible `docs/runbooks/s3-mirror.md`.
- **Knowingly not covered**: RBD volumes, CephFS subvolumes, and buckets of any stage but prd — including every bucket on the dev cluster's Ceph (slice 022 rulings T2, T3).
