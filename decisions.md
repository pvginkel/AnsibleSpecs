...
- **47-day certs, 14-day renewal threshold.** Same shape as the TLS leaves, schedule included: the weekly `iac-scheduled-certs` job runs `playbooks/renew-host-certs.yml`, and `ssh_host_cert` re-signs when the cert is within the threshold, otherwise no-op.
