...
- **KubeCoder's own provisioner password is the exception, and lives in OpenBao.** KubeCoder drives this CA through a JWK provisioner of its own — `kubecoder-jwk`, separate key and separate password, never the fleet's — and that password is materialised into the cluster by ESO from `eso/prd/kubecoder/<stage>/step-ca-provisioner-password` rather than kept in ansible-vault. The bullet above rules that out for `ansible-jwk`, but its reason is bootstrap ordering: OpenBao's own listener cert is issued through that provisioner, so that password must be available before OpenBao is. KubeCoder is not in the whole-site bring-up order (§OpenBao backup / DR), so the reason does not reach it. Recorded as an explicit exception rather than left to land silently; the bootstrap-tier rule is unchanged and `ansible-jwk`'s password stays out of OpenBao.
