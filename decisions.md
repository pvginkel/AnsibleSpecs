...
- **Leaf cert validity: 47 days**, all consumers. cert-manager handles its own renewal cadence in-cluster; the `internal_tls` role re-issues when remaining validity drops below 14 days. Prometheus alert per consumer at `<17` days remaining so a silent renewer failure surfaces well before expiry. VM-side expiry metric comes from a node-exporter textfile collector that the `internal_tls` role updates after each issue; in-cluster expiry comes from cert-manager's built-in exporter.
...
- **Pending — CNAME support in the dnsmasq `static-hosts.yaml` tooling.** An alias hostname (e.g. `kubernetes-api-dev.home` → `wrkdevk8s`) is a duplicated A record today, repeating the target's IP. CNAME entries would let an alias point at the canonical `hostname.home` so the address stays in one place. Not needed yet; first consumer is the `kubernetes-api-dev.home` alias (`slices/internal-ha-vips.md` §E).
