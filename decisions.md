...
- **TLS**: per-node leaf cert; SANs cover the node's short hostname, FQDN, and `openbao.home` (the VIP). Issued by the homelab `step-ca` via ACME, 47-day validity, renewed daily by `certbot` on each srvvaultN. See "Internal TLS / homelab CA" below for the broader CA design.
...
Homelab-wide CA for internal-DNS TLS. Stands up in Phase 2 because OpenBao is the first consumer; expands to PVE and the Kubernetes API server in the same rollout.

- **`step-ca`** (Smallstep), Helm-deployed from `HelmCharts`. Chosen for native ACME support — every leaf is issued via the same protocol the existing public-cert flow already uses (`certbot/certbot` container + Flask shim), so internal certs reuse the existing tooling with only a directory URL change.
...
- **Intermediate key + passphrase: regular Kubernetes Secret**, not delivered via ESO from OpenBao. The chicken-and-egg of OpenBao-needs-cert-from-step-ca-needs-passphrase-from-OpenBao is not worth the marginal security gain when short leaf validity already bounds blast radius.
- **Leaf cert validity: 47 days**, all consumers. Daily ACME renewal via `certbot` on each consumer; certbot's default 1/3-remaining threshold fires renewal around day 32 of life. Prometheus alert per consumer at `<17` days remaining so a silent renewer failure surfaces well before expiry.
...
- **OpenBao bootstrap**: srvvaultN's first cert is fetched via `step ca certificate` (HTTP-01 challenge over the homelab LAN) at role-apply time. No self-signed bootstrap step. Cold-boot of an existing srvvaultN uses the cert from disk; only renewal needs step-ca, so a step-ca / k8s outage shorter than 47 days is invisible to OpenBao consumers. Beyond 47 days the cert expires and OpenBao becomes unreachable until either step-ca returns or a cert is replaced manually.
- **Initial scope (v1)**: OpenBao listener certs (3 nodes), PVE Web UI / API certs (3 hosts), Kubernetes API server cert. These are the internal HTTPS endpoints the operator hits routinely. Everything else (dnsmasq UI, in-cluster service endpoints, IoT, printers, etc.) stays on snakeoil / self-signed until later sweeps; in-cluster services migrate via cert-manager + a step-ca ACME ClusterIssuer when those services get touched anyway.
