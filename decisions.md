...
- A Kubernetes namespace outlives any single chart in it (reinstalls, multiple subcharts). TF owns the namespace; Helm deploys into it. For an app Argo CD deploys the Application is the unit instead: the namespace is a tracked manifest in the app's own chart, and the app's Terraform never manages it — the PreSync hook is granted no `namespaces` ([`argo-cd/decisions.md`](argo-cd/decisions.md) D25, D33).
...
- **Nine out-of-repo copies of the same file exist**, all byte-identical to the Ansible canonical copy — `/work/HelmCharts/homelab-root.crt`, `/work/HelmCharts/charts/nginx/files/ca/homelab-root.crt`, `/work/ArgoCDTools/argocd-hook/image/homelab-root.crt` (the Argo CD presync hook image), `/work/ArgoCDTools/aac-tools/image/homelab-root.crt` (the `aac-tools` image), `/work/DockerImages/kube-coder-dev-base/homelab-root.crt`, `/work/DockerImages/kube-coder-arm64-cross-toolchain/homelab-root.crt`, `/work/DockerImages/kube-coder-esp-idf-toolchain/homelab-root.crt`, `/work/ArgoCDDeploy/chart/files/homelab-root.crt` (mounted into Argo CD's repo-server so `helm dependency build` trusts `https://charts.home`) and `/work/KubeCoderDeploy/chart/files/ca/homelab-root.crt` (KubeCoder's controller CA mount once KubeCoder deploys from Argo). HelmCharts' `charts/jenkins/` and `charts/kubecoder/` reach the root through symlinks to the repo-root copy, so they are not separate copies and a rotation does not touch them. A rotation that updates the in-repo bundle here must update all nine in the same change window, ordered *before* the new root starts appearing in step-ca's `/roots.pem` (otherwise the drift pipeline fires until the others catch up); the five image copies additionally need their images rebuilt to take effect, and the ArgoCDDeploy copy needs a sync of Argo's own chart, which is manual by design. Eventual fix is to collapse the duplication — single source via submodule, build-time fetch from this repo, or moving the canonical copy under HelmCharts and having Ansible read from there — recorded as a follow-up rather than blocking the next rotation.
- **The KubeCoder controller keeps no copy of its own.** It validates `https://ca.home` when it asks step-ca to sign an environment pod's SSH host certificate, and its base image (`python:slim`) carries stock Debian roots only — so the root reaches it as a chart mount rather than through the image's trust store: HelmCharts' `kubecoder` chart renders the repo-root copy into a ConfigMap and hands the pod the mounted path in `KUBECODER_STEP_CA_ROOT`, and `/work/KubeCoderDeploy/chart/files/ca/homelab-root.crt` takes that over once KubeCoder deploys from Argo. A rotation lands there by a chart deploy, with no image rebuild. The controller image did bake its own `/work/KubeCoder/controller/homelab-root.crt` until September 2026; that file is gone and the Dockerfile no longer copies it.
...
**HelmCharts' deploy-time digest resolution is not a pin in that sense.** Its deploy tooling (`resolve_helm_args.py`, which `migrate-release.py --no-pin` turns off) rewrites every image reference in a chart's templates to `@sha256:` at deploy time, and the `Jenkinsfile` uses the resolved digest as the signal that a release changed and has to be deployed. That resolution follows whatever the written tag points at *now* — it fixes what one deploy runs, not what a later one will — where a pin above is a reference committed here that an upstream tag move cannot shift. The two uses of the word are different mechanisms; only the first gates anything.
...
- **Off-cluster copies of production data**, each on Google Drive under `gdrive-pieter:Homelab Backups`:
  ...
  - *`postgres-pas` databases* — the `postgres-backup` CronJob's nightly `pg_dump` of each application database through `backup-server`, scope `postgres-pas`, 90 newest uploads kept (HelmCharts `configs/prd/postgres-pas/_shared/infrastructure.tf`); the same age key. `BackupOverdue` fires critical for a database after 52 h without a new dump of it; a database dropped on purpose alerts until pruning removes its last dump or its `.metadata.json` files are deleted by hand. Triage: Ansible `docs/runbooks/backup-freshness.md`.
  - *YouTrack* — the `youtrack-backup` CronJob in `youtrack-prd`, nightly: YouTrack writes its own database backup (issues, attachments, the built-in Hub), which the job starts and downloads over the REST API as the `backup` service user (token in OpenBao `eso/prd/youtrack/prd/backup`) and posts through `backup-server`, scope `youtrack`, 30 newest uploads kept (HelmCharts `configs/prd/youtrack/_shared/infrastructure.tf`); the same age key. Not the RBD volume: the embedded database cannot be copied while open, and the volume is `ReadWriteOnce`. Its uploads declare no validity, so `BackupOverdue` does not watch it; `YouTrackBackupStale` fires critical after 52 h without a successful run of the CronJob. Restore and drill: Ansible `docs/runbooks/youtrack-restore.md`.
