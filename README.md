...
- **[019](slices/backlog/019_role_followups_kubelite_tls_openbao_backup/slice.md)** — Ansible role follow-ups: the OpenBao backup can install a dead credential and its wrapper never says which call failed, the kubelite restart is invisible under `--check` and its wait checks liveness, and an expired `internal_tls` leaf has no recovery runbook (major; #573 items 1–2, 016 close-out B3/S2/B10; #573 item 3 split to 023 at planning).
